...
<div {{ $attributes->root() }} x-data="{
    ...
    <x-senna.dropdown {{ $attributes->namespace('dropdown')->merge(['align' => 'left']) }}>
        <x-slot name="trigger">
            <div {{ $attributes->namespace('trigger')->merge(['class' => "cursor-pointer text-primary inline-flex gap-1 items-center justify-center"]) }} x-show="options.length > 1" class="">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" {{ $attributes->namespace('icon')->merge(['class' => "w-3 h-3 shrink-0"]) }}>
        ...
        <x-slot name="content" class="flex flex-col gap-2 max-h-[266px] overflow-y-auto">
