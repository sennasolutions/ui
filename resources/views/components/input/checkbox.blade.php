...
<label {{ $attributes->merge(['class' => 'sn-input-check cursor-pointer inline-flex space-x-2 items-center'])->only(['class', 'wire:key']) }}>
    <input type="checkbox" class="cursor-pointer transition duration-50 ease-in-out rounded checked:border-none border-gray-300 w-[1.15rem] h-[1.15rem] text-primary-color shadow-sm focus:border-primary-color-50 focus:ring focus:ring-primary-color-30 {{ $inputClass }}" {!! $attributes->except(['class', 'wire:key']) !!}>
